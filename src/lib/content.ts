...
export async function getRecentChangelogEntries(
  ...
  return (await getChangelogEntries()).filter(
    (entry) => new Date(`${entry.date}T00:00:00Z`).getTime() >= threshold,
  );
}
